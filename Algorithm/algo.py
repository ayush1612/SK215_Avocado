...
def forward(place):
    forward_url = "https://api.mapbox.com/geocoding/v5/mapbox.places/"+ place + ".json?access_token=pk.eyJ1IjoidWp3YWxrcGwiLCJhIjoiY2tkYTdvZG1kMGJlMjJybXpvaHJ0NDRieiJ9.mx5YB4-2ZKCXsLzOpC--og"
...
def reverse(cor_long,cor_lat):
    URL_Reverse = "https://api.mapbox.com/geocoding/v5/mapbox.places/"+str(cor_long)+","+str(cor_lat)+".json?access_token=pk.eyJ1IjoidWp3YWxrcGwiLCJhIjoiY2tkYTdvZG1kMGJlMjJybXpvaHJ0NDRieiJ9.mx5YB4-2ZKCXsLzOpC--og"
...
def path(from_lat,from_long,to_lat,to_long):
    URL_Route = "https://api.mapbox.com/directions/v5/mapbox/cycling/"+str(from_lat)+","+str(from_long)+";"+str(to_lat)+","+str(to_long)+"?geometries=geojson&access_token=pk.eyJ1IjoidWp3YWxrcGwiLCJhIjoiY2tkYTdvZG1kMGJlMjJybXpvaHJ0NDRieiJ9.mx5YB4-2ZKCXsLzOpC--og"
...
def priority(locality):
    dict = {"mathikere":2,"malleswaram":3,"yesvantpur":2,"hebbal":3,"peenya":3,"sanjaynagar":1,"MSR Nagar":2,"Armane nagar":1,"RMV stage 2":2,"sadashiva nagar":1}
    priority = dict.get(locality,1)
    return priority
@app.route('/')
def home():
	return "hello"
...
def predict():
    ...
    for i in coordinates:
        ...
        if locality not in police_path:
            ...
            print(locality + " " + str(priority(locality)))
